	...
	while (true) {
		...
		for (int i = 0; i < numItems; ++i) {
			...
			if (laborLevel >= targetLevel) {
				...
				// [수정] 단가 뒤에 "/ 일 n회" 형식으로 횟수 감소를 명확하게 표시
...
void GameManager::processNextDay() {
	...
	if (!debt.nextDay()) {
		system("cls");
		std::cout << WIDTH_BORDER << std::endl;
		std::cout << "\n  [ 게임 오버 ] 50일의 기한이 모두 지났습니다..." << std::endl;
		std::cout << "  결국 빚을 청산하지 못하고 사채업자들에게 끌려갑니다.\n" << std::endl;
		std::cout << WIDTH_BORDER << std::endl;
		(void)_getch();
		isGameOver = true;
		return;
	...
	if (debt.getAccumulatedInterest() >= 100000000) {
		...
		std::cout << WIDTH_BORDER << std::endl;
		std::cout << "\n  [ 파산 / 게임 오버 ]" << std::endl;
		std::cout << "  누적 이자가 1억 원을 돌파했습니다..." << std::endl;
		std::cout << "  눈덩이처럼 불어난 빚을 감당하지 못해 완전히 파산했습니다.\n" << std::endl;
		std::cout << WIDTH_BORDER << std::endl;
		(void)_getch();
		isGameOver = true;
		return;
...
void GameManager::checkGameEndConditions() {
	if (debt.getTotalDebt() <= 0) {
		...
		std::cout << WIDTH_BORDER << std::endl;
		std::cout << "\n  [ 게임 클리어! ]" << std::endl;
		std::cout << "  모든 빚을 청산했습니다! 도박의 늪에서 벗어나 새 삶을 시작합니다!\n" << std::endl;
		std::cout << "  최종 남은 자산: " << formatToKorean(player.getMoney()) << std::endl;
		std::cout << WIDTH_BORDER << std::endl;
		(void)_getch();
		isGameClear = true;
	}
	...
	while (true) {
		...
		if (mainChoice == 0) {
			...
			while (!isGameOver && !isGameClear) {
				...
				else if (action == 2) jobMgr.runMenu(player);
				else if (action == 3) upgradeMgr.runMenu(player);
				else if (action == 4) {
